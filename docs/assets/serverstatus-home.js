...
.then(data => {
    if (data.online == true) {
        ...
        document.getElementById("serverPlaynow").innerHTML = `<span class="inline-block bg-yellow-500 rounded-full px-3 py-1 text-sm font-thin uppercase text-gray-900 mr-2 shadow-lg">Почати Гру: ${data.players.online} ${data.motd.clean} Онлайн</span>`;
        {% else %}
